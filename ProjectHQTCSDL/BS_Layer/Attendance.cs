...
namespace ProjectHQTCSDL.BS_Layer
{
    public class Attendance
    {
        ...
        {
            DataTable dt = connectData.ExcuteQuery("SELECT * FROM DanhSachLopTheoBuoi (" + idClass + ", " + session + ")", ref error);
            if (dt != null)
            {
                dt.Columns.Add("Absent", typeof(bool));
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    if (CheckAbsent(idClass, session, int.Parse(dt.Rows[i]["MaHocVien"].ToString()), connectData))
        ...
        public string InsertAbsent (List<int> iDStudent, string iDClass, string session, dbMain connectData)
        {
            ...
            {
                connectData.ExcuteNonQuery("EXECUTE ThemBuoiVang " + iDStudent[i] + ", " + iDClass + ", " + session, ref err);
            }
            return err;
        }
    }
